...
void MagneticSensor_Init(float zero_electric_offset, Direction _sensor_direction)
{
    ...
    delay(50);
    shaftVelocity = getShaftVelocity(); // 必须调用一次，进入主循环后速度为0
    delay(5);
...
{
    return _normalizeAngle((shaftAngle + sensor_offset) * polePairs - zero_electric_angle);
}

// float getElectricalAngle()
// {
//     // If no sensor linked return previous value (for open-loop case)
//     return _normalizeAngle((float)(countDirection * polePairs) * getLapAngle() - zero_electric_angle);
// }
