...
void printLog()
{
  ...
  // sprintf(sndBuff, "velocity=%.2f,P=%.2f,I=%.2f,D=%.2f,PID_ouput:%.2f\n", shaftVelocity, comm1, comm2, comm3, comm4);
  // HAL_UART_Transmit_DMA(&huart3, (uint8_t *)sndBuff, sizeof(sndBuff));
  // printf("velocity=%.2f, RPM=%d\n", shaftVelocity, getRPM());
  //printf("velocity=%.2f\n", shaftVelocity);
  // printf("ShaftAngle=%.2f\n", getShaftAngle());
...
void commander_run(void)
{
  if (rxUart.toProcessData == 1)
  {
    ...
    case 'T': // T6.28
      ...
      break;
    case 'P': // P0.5
      PID_velocity.P = atof((const char *)(rxUart.buf + 1));
      sprintf(sndBuff, "P=%.2f\r\n", PID_velocity.P);
      printf("%s", sndBuff);
      break;
    case 'I': // I0.2
      PID_velocity.I = atof((const char *)(rxUart.buf + 1));
