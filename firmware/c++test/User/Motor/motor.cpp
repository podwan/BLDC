...
{
    // If no sensor linked return previous value (for open-loop case)
    return encoder ? Normalize((float)(encoder->countDirection * polePairs) * encoder->GetLapAngle() - zeroElectricAngleOffset)
                   : electricalAngle;
}
...
bool Motor::AlignSensor()
{
    if (encoder->countDirection == EncoderBase::UNKNOWN)
    {
        ...
        {
            encoder->countDirection = EncoderBase::CCW;
        }
        else
        {
            encoder->countDirection = EncoderBase::CW;
        }
...
void Motor::CloseLoopControlTick()
{
    ...
    if (config.controlMode != ANGLE_OPEN_LOOP &&
        config.controlMode != VELOCITY_OPEN_LOOP)
    {
        estimateAngle = GetEstimateAngle();
    }
